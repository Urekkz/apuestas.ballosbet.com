...
class Elementor {
	...
	public function __construct() {
		...
		add_action( 'jet-form-builder/form-handler/after-send', [ $this, 'elementor_clear_cache' ] );
	...
	public function elementor_clear_cache( $clear_cache = true ) {
		...
		if ( class_exists( '\Elementor\Core\Files\CSS\Post' ) && method_exists( '\Elementor\Core\Files\CSS\Post', 'create' ) ) {
			foreach ( $posts as $post_id ) {
				try {
					\Elementor\Core\Files\CSS\Post::create( $post_id )->update();
				} catch ( \Throwable $e ) {
					if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
						error_log( 'Elementor CSS regen error for post ' . $post_id . ': ' . $e->getMessage() );
					}
				}
			}
		}
