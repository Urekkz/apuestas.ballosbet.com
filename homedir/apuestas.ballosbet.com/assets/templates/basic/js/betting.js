(function ($) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.status === 'success') {
                            ...
                            const newBalance = getUserBalance() - selectedAmount;
                            updateBalanceDisplay(newBalance);
                            
                            // Actualizar el panel de "Mis Apuestas"
                            if (response.html) {
                                $('.mybet-container').html(response.html);
                            }
                            
                            // Cambiar al tab de "Mis Apuestas"
                            $('#mybets-btn').prop('checked', true);
                            ...
                            // Recargar la página DESPUÉS de un tiempo más largo para permitir UI updates
    ...
    $(document).on('click', '.btn-match-bet', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Actualizar "Mis Apuestas" si viene HTML
                if (response.html) {
                    $('.mybet-container').html(response.html);
                }

                // Remover la tarjeta de apuesta tapada
                $btn.closest('.open-bet-card').fadeOut(300, function() {
                    $(this).remove();

                    // Si no quedan más apuestas, mostrar mensaje vacío
                    const $panel = $btn.closest('.side-panel-body');
                    if ($panel.find('.open-bet-card').length === 0) {
                        $panel.html(`
    ...
    // Función para recargar las apuestas abiertas (opcional)
    window.refreshOpenBets = function() {
        console.log('🔄 Recargando apuestas abiertas...');
        
        // Recargar la sección de apuestas abiertas
        $.ajax({
            url: window.location.href,
            method: 'GET',
            success: function(html) {
                const $newContent = $(html).find('.open-bets-section');
                if ($newContent.length) {
                    $('.open-bets-section').replaceWith($newContent);
                    console.log('✅ Apuestas abiertas actualizadas');
                }
            },
            error: function(xhr) {
                console.error('❌ Error al recargar apuestas:', xhr);
            }
        });
    };

    // Auto-refrescar apuestas abiertas cada 30 segundos
    setInterval(window.refreshOpenBets, 30000);
