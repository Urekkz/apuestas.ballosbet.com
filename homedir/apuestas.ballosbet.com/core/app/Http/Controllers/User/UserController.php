    ...
    public function __construct()
{
    $this->middleware(function ($request, $next) {
        ...
        // Si no hay usuario logueado, o su status no es 1, redirigir al login
        if (!$user || $user->status != 1) {
            auth()->logout();
            return redirect()->route('user.login')
                ->with('error', 'Tu cuenta no está activa. Contacta soporte.');
        }
    ...
    /**
     * Return the authenticated user's balance as JSON (formatted and raw).
     */
