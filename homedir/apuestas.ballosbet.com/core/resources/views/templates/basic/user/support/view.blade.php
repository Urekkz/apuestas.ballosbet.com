            ...
            <div class="d-flex flex-warp justify-content-between align-items-center mb-3">
                ...
                @if ($myTicket->status != Status::TICKET_CLOSE && $myTicket->user)
                    <button class="btn btn-sm btn--danger" data-bs-toggle="modal" data-bs-target="#ticketCloseModal" type="button">
                        <i class="las la-times"></i> @lang('Cerrar Ticket')
                    </button>
                @endif
            </div>
                ...
                <div class="row justify-content-between">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label class="form-label">@lang('Tu respuesta')</label>
                            <textarea name="message" class="form-control form--control" rows="4" required>{{ old('message') }}</textarea>
                        </div>
                    </div>

                    <div class="col-md-9">
                        <button type="button" class="btn btn-dark btn-sm addAttachment my-2"> <i class="fas fa-plus"></i> @lang('Adjuntar Archivo') </button>
                        <div class="mb-2"><small class="text-muted"> <i class="la la-info-circle"></i> @lang('Se pueden subir un máximo de 5 archivos | El tamaño máximo de carga es' . convertToReadableSize(ini_get('upload_max_filesize')) . ' | Allowed File Extensions: .jpg, .jpeg, .png, .pdf, .doc, .docx')</small></div>
                        <div class="row fileUploadsContainer"></div>
                    </div>
                    <div class="col-md-3">
                        <button class="btn btn--base w-100 my-2" type="submit"><i class="la la-fw la-lg la-reply"></i> @lang('Responder')
                        </button>
                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <h6>@lang('¿Estás seguro de que quieres cerrar este ticket??')</h6>
                    <div class="d-flex gap-3">
                        ...
                        <form action="{{ route('ticket.close', $myTicket->id) }}" method="POST" class="flex-grow-1">
                            @csrf
                            <button class="btn btn--base w-100" type="submit">@lang('Sí')</button>
                        </form>
                    </div>
                </div>
            </div>
