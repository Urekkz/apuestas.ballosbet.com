...
<div class="open-bets-section mt-4">
    ...
    <div class="row g-3">
        {{-- Columna Izquierda (Gallo 1 / Outcome 1) --}}
        <div class="col-md-6">
            <div class="side-panel left-side">
                <div class="side-panel-header">
                    <span class="side-label">{{ __($outcomeOne?->name ?? 'Izquierda') }}</span>
                </div>
                <div class="side-panel-body">
                    ...
                    @if($leftBets->isNotEmpty())
                        ...
                        @foreach($leftBets as $bet)
                            ...
                            @endphp
                            <div class="col">
                            <div class="open-bet-card {{ $isMatched ? 'matched' : '' }} h-100">
                                @if($isMatched)
                                ...
                                <div class="bet-info">
                                    ...
                                    <div class="bet-amount">
                                        ...
                                        <span class="value">{{ showAmount($bet->stake_amount) }}</span>
                                    </div>
                                </div>
                                @if($canMatch)
                                    <button
                                        ...
                                        data-bet-id="{{ $bet->id }}"
                                        data-amount="{{ $bet->stake_amount }}"
                                        data-opposite-outcome-id="{{ $outcomeTwo->id }}"
                                        data-opposite-outcome-name="{{ $outcomeTwo->name }}"
                                        data-odds="{{ $outcomeTwo->odds }}"
                                    >
        ...
        {{-- Columna Derecha (Gallo 2 / Outcome 2) --}}
        <div class="col-md-6">
            <div class="side-panel right-side">
                <div class="side-panel-header">
                    <span class="side-label">{{ __($outcomeTwo?->name ?? 'Derecha') }}</span>
                </div>
                <div class="side-panel-body">
                    @php
                    ...
                    @if($rightBets->isNotEmpty())
                        <div class="row row-cols-2 row-cols-md-3 g-2 open-bets-grid">
                        @foreach($rightBets as $bet)
                            @php
                                $betItem = $bet->bets->first();
                                $canMatch = auth()->check() && auth()->id() != $bet->user_id && !$bet->matched_bet_id;
                                $isMatched = $bet->matched_bet_id != null;
                            ...
                            <div class="open-bet-card {{ $isMatched ? 'matched' : '' }} h-100">
                                ...
                                @if($canMatch)
                                    <button
                                        ...
                                        data-bet-id="{{ $bet->id }}"
                                        data-amount="{{ $bet->stake_amount }}"
                                        data-opposite-outcome-id="{{ $outcomeOne->id }}"
                                        data-opposite-outcome-name="{{ $outcomeOne->name }}"
                                        data-odds="{{ $outcomeOne->odds }}"
                                    >
                                        <i class="las la-hand-rock"></i> Tapar Apuesta
                                    </button>
                                @elseif($isMatched)
