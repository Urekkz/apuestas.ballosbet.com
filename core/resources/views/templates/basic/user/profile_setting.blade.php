...
@section('master')
        ...
        <div class="card-body">
            ...
            <ul class="list-group d-flex flex-column flex-md-row flex-wrap gap-3 justify-content-between">
                <li class="list-group-item d-flex flex-column border-0 p-0">
                    <small class="text-muted">@lang('Nombre de usuario')</small>
                    <span class="fw-semibold lh-1"> {{ $user->username }}</span>
                </li>
                <li class="list-group-item d-flex flex-column border-0 p-0">
                    <small class="text-muted">@lang('Correo')</small>
                    <span class="fw-semibold lh-1"> {{ $user->email }}</span>
                </li>
                <li class="list-group-item d-flex flex-column border-0 p-0">
                    <small class="text-muted">@lang('Celular')</small>
                    <span class="fw-semibold lh-1"> {{ $user->mobile }}</span>
                </li>

                <li class="list-group-item d-flex flex-column border-0 p-0">
                    <small class="text-muted">@lang('País')</small>
                ...
                <li class="list-group-item d-flex flex-column border-0 p-0">
                    ...
                    @if ($user->kv)
                        <span class="fw-semibold lh-1 text--success"> <i class="la la-check-circle"></i> @lang('Verificado')</span>
                    @else
                        <span class="fw-semibold lh-1 text--danger"> <i class="la la-times-circle"></i> @lang('Sin Verificar')</span>
                    @endif
                </li>
            </ul>
    ...
    <div class="mb-3">
        <h5 class="m-0">@lang('Actualiza tu perfil')</h5>
        <small class="text-muted">
            @lang('Mantén tu información actualizada para garantizar una comunicación fluida y una gestión eficaz de tu cuenta. Revisa y actualiza tus datos a continuación.')
        </small>
    </div>
    ...
    <form class="register" method="post" enctype="multipart/form-data">
        ...
        <div class="card custom--card">
            <div class="card-body">
               <h6 class="m-0 mb-2">@lang('Información Persolnal')</h6>
                <div class="row">
               ...
               <h6 class="mt-3 mb-2">@lang('Información de contacto')</h6>
                <div class="row">
                    <div class="form-group col-sm-6">
