<div class="header-fluid-custom-parent">
    ...
    <nav class="primary-menu-container">
        ...
        <ul class="align-items-center justify-content-start gap-3 gap-lg-4 mb-0 p-0 list-unstyled d-none d-lg-flex main-nav-menu">
            <li>
                <a href="https://apuestas.ballosbet.com/" class="nav-link-item {{ request()->is('/') ? 'active' : '' }}">
                    @lang('INICIO')
                </a>
            </li>
            <li>
                <a href="https://ballosbet.com/eventos" class="nav-link-item {{ request()->is('eventos*') ? 'active' : '' }}">
                    @lang('EVENTOS')
                </a>
            </li>
            <li>
                <a href="https://ballosbet.com/resultados-2" class="nav-link-item {{ request()->is('resultados*') ? 'active' : '' }}">
                    @lang('RESULTADOS')
                </a>
            </li>
            <li>
                <a href="https://ballosbet.com/videoteca" class="nav-link-item {{ request()->is('videoteca*') ? 'active' : '' }}">
                    @lang('VIDEOTECA')
                </a>
            </li>
            <li>
                <a href="https://ballosbet.com/membresias" class="nav-link-item {{ request()->is('membresias*') ? 'active' : '' }}">
                    @lang('MEMBRESÍAS')
                </a>
            </li>
        </ul>
        ...
        <ul class="list list--row primary-menu justify-content-end align-items-center right-side-nav gap-3 gap-sm-4">
            ...
            <li class="d-flex align-items-center">
                @auth
                    <div class="app-nav__menu-text text-white ps-2">@lang('Saldo'): <strong id="mobileBalance">{{ showAmount(auth()->user()->balance) }}</strong></div>
                @endauth
            </li>
            ...
            <li class="d-none d-lg-block">
                ...
                @else
                    @if (Route::is('user.login'))
                        <a class="btn btn--signup" href="{{ route('user.register') }}"> @lang('Ingresar') </a>
                    @else
